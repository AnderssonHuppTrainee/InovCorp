<template>
    <AppLayout>
        <div class="space-y-6 p-4">
            <PageHeader title="Detalhes da Ação" :description="calendarAction.name">
                ...
                <Card>
                    ...
                    <CardContent class="space-y-4">
                        <div>
                            ...
                            <div class="mt-1 text-sm">{{ calendarAction.name }}</div>
                        </div>

                        <div>
                            <div class="text-sm font-medium text-muted-foreground">Descrição</div>
                            <div class="mt-1 text-sm">{{ calendarAction.description || '-' }}</div>
                        </div>

                        <div>
                            <div class="text-sm font-medium text-muted-foreground">Estado</div>
                            <div class="mt-1">
                                <Badge :variant="calendarAction.is_active ? 'default' : 'secondary'">
                                    {{ calendarAction.is_active ? 'Ativa' : 'Inativa' }}
                                </Badge>
                            </div>
                        </div>
                    </CardContent>
                ...
                <Card>
                    ...
                    <CardContent class="space-y-4">
                        ...
                        <div class="pt-4">
                            ...
                            <div class="mt-1 text-sm">{{ formatDate(calendarAction.created_at) }}</div>
                        </div>

                        <div>
                            <div class="text-sm font-medium text-muted-foreground">Atualizado em</div>
                            <div class="mt-1 text-sm">{{ formatDate(calendarAction.updated_at) }}</div>
                        </div>
                    </CardContent>
                </Card>
            </div>
...
const handleEdit = () => {
    router.visit(calendarActions.edit({ calendarAction: props.calendarAction.id }).url)
}

const handleDelete = () => {
    if (confirm('Tem certeza que deseja eliminar esta ação?')) {
        router.delete(calendarActions.destroy({ calendarAction: props.calendarAction.id }).url)
    }
}
</script>

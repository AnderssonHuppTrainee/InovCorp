...
const handleEdit = () => {
    router.visit(contactRoles.edit({ contact_role: props.contactRole.id }).url);
};
...
const handleDelete = () => {
    if (confirm('Tem certeza que deseja eliminar esta função?')) {
        router.delete(
            contactRoles.destroy({ contact_role: props.contactRole.id }).url,
        );
    }
};
</script>
